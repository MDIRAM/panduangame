<form wire:submit.prevent="{{ $step === 1 ? 'sendOtp' : 'register' }}" class="auth-form">
    @if ($statusMessage)
    ...
    <div>
        ...
        <input wire:model="name" id="name" type="text" placeholder="Nama lengkap" required {{ $step === 2 ? 'disabled style=opacity:0.6;' : 'autofocus' }} />
        @error('name')
    ...
    <div>
        ...
        <input wire:model="email" id="email" type="email" placeholder="you@example.com" required {{ $step === 2 ? 'disabled style=opacity:0.6;' : '' }} />
        @error('email')
            <span class="auth-error">{{ $message }}</span>
        @enderror
    </div>
    ...
    @if ($step === 2)
        <div style="animation: fadeIn 0.4s ease-out; margin-top: 1.25rem;">
            <label for="otp_code">OTP Verification Code (4-Digit)</label>
            <input wire:model="otp_code" id="otp_code" type="text" maxlength="4" placeholder="OTP" required autofocus style="text-align: center; font-size: 1.3rem; letter-spacing: 6px; font-weight: 800;" />
            @error('otp_code')
                <span class="auth-error">{{ $message }}</span>
            @enderror
        </div>
